...
static void start_task(void* arg)
{
    ...
    if (init_ret != OM_OK)
    {
        system_health_report_fatal(SYSTEM_HEALTH_ERR_BSP_INIT_FAIL, "bsp_register_all failed");
    ...
    {
        system_health_report_fatal(SYSTEM_HEALTH_ERR_EVENT_BUS_INIT_FAIL, "event_bus_init failed");
        goto supervisor_loop;
    }
    ...
    if (imu_init_ret != 0U)
    {
        system_health_report_fatal(SYSTEM_HEALTH_ERR_MPU_DEVICE_INIT_FAIL, "mpu_device_init failed");
        goto supervisor_loop;
    }
    ...
    {
        system_health_report_fatal(SYSTEM_HEALTH_ERR_IMU_TASK_START_FAIL, "imu_task_start failed");
        goto supervisor_loop;
    }
    ...
    {
        system_health_report_fatal(SYSTEM_HEALTH_ERR_INPUT_TASK_START_FAIL, "input_task_start failed");
        goto supervisor_loop;
    }
    ...
    {
        system_health_report_fatal(SYSTEM_HEALTH_ERR_MODE_TASK_START_FAIL, "mode_task_start failed");
        goto supervisor_loop;
    }

    motor_communications_task_ret = motor_communications_task_start(devices);
    if (motor_communications_task_ret != OM_OK)
    {
        system_health_report_fatal(
            SYSTEM_HEALTH_ERR_MOTOR_COMMUNICATIONS_TASK_START_FAIL,
            "motor_communications_task_start failed");
        goto supervisor_loop;
    }

    if (system_health_register(
            SYSTEM_HEALTH_TASK_MOTOR_COMMUNICATIONS,
            50u,
            SYSTEM_HEALTH_ERR_MOTOR_COMMUNICATIONS_TIMEOUT) != OM_OK)
    {
        system_health_report_fatal(
            SYSTEM_HEALTH_ERR_SYSTEM_HEALTH_REGISTER_FAIL,
            "system_health_register MOTOR_COMMUNICATIONS failed");
        goto supervisor_loop;
    }
    ...
    {
        system_health_report_fatal(
            SYSTEM_HEALTH_ERR_CHASSIS_TASK_START_FAIL,
    ...
    if (system_health_register(
            ...
            50u,
            SYSTEM_HEALTH_ERR_CHASSIS_TASK_TIMEOUT) != OM_OK)
    {
        system_health_report_fatal(
            SYSTEM_HEALTH_ERR_SYSTEM_HEALTH_REGISTER_FAIL,
            "system_health_register CHASSIS failed");
        goto supervisor_loop;
    }
    ...
    {
        system_health_report_fatal(
            SYSTEM_HEALTH_ERR_ARM_TASK_START_FAIL,
    ...
    if (system_health_register(
            ...
            50u,
            SYSTEM_HEALTH_ERR_ARM_TASK_TIMEOUT) != OM_OK)
    {
        system_health_report_fatal(
            SYSTEM_HEALTH_ERR_SYSTEM_HEALTH_REGISTER_FAIL,
            "system_health_register ARM failed");
        goto supervisor_loop;
    }
    ...
    {
        system_health_report_fatal(SYSTEM_HEALTH_ERR_VOFA_TASK_START_FAIL, "vofa_task_start failed");
        goto supervisor_loop;
    }
