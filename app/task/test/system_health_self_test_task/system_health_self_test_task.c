...
#define SYSTEM_HEALTH_SELF_TEST_TASK_PERIOD_MS (20u)

static void system_health_self_test_task_entry(void* arg)
{
    ...
    while (1)
    {
        ...
        if (fault_injected != OM_TRUE && elapsed_ms >= APP_SYSTEM_HEALTH_SELF_TEST_DELAY_MS)
        {
            ...
            system_health_report_fatal(SYSTEM_HEALTH_ERR_SELF_TEST_FATAL, "system_health self-test fatal injected");
            fault_injected = OM_TRUE;
#else
            return;
#endif
        ...
        {
            (void)system_health_beat(SYSTEM_HEALTH_TASK_SELF_TEST);
            (void)osal_delay_until(&deadline_cursor_ms, SYSTEM_HEALTH_SELF_TEST_TASK_PERIOD_MS, OM_NULL);
            continue;
        }
...
OmRet system_health_self_test_task_start(void)
{
    ...
    const OsalThreadAttr self_test_task_attr = {"system_health_self_test", 512u * OSAL_STACK_WORD_BYTES, 3u};
    ...
    status = osal_thread_create(&self_test_task_thread, &self_test_task_attr, system_health_self_test_task_entry, OM_NULL);
    if (status != OSAL_OK)
    {
