 ...
 */
const unsigned short crc_ccitt_false_table[256] = {
 ...
 */
inline uint16_t crc_ccitt(uint8_t /*const*/ *buffer, size_t len)
{
	uint16_t crc = 0;
	while (len--)
 ...
 *	@len: number of bytes in the buffer
 */
uint16_t crc_ccitt_false(uint16_t crc, uint8_t const* buffer, uint32_t len)
{
	while (len--)
